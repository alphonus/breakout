...
task runr, "Runs breakout for current platform":
 exec &"nim c -r {releaseOpts} -o:breakout src/main.nim"

task rund, "Runs debug breakout for current platform":
 exec &"nim c -r {debugOpts} -o:breakout src/main.nim"

task release, "Builds breakout for current platform":
 exec &"nim c {releaseOpts} -o:breakout src/main.nim"

task webd, "Builds debug breakout for web":
 exec &"nim c {debugOpts} -d:emscripten -o:breakout.html src/main.nim"

task webr, "Builds release breakout for web":
 exec &"nim c {releaseOpts} -d:emscripten -o:breakout.html src/main.nim"

task debug, "Builds debug breakout for current platform":
 exec &"nim c {debugOpts} -o:breakout_debug src/main.nim"
